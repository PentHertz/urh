...
class TestAutoAssignments(unittest.TestCase):
    def setUp(self):
        ...
        self.decodings.append(
            Encoding(
                ["DeWhitening", settings.DECODING_DATAWHITENING, "0x67686768;0x21;0"]
            )
        )
    ...
    def test_message_type_assign_by_value(self):
        ...
        msg_type.ruleset = Ruleset(
            Mode.all_apply, [Rule(start, end, "=", hex_value, 1)]
        )
        ...
        for i, message in enumerate(self.protocol.messages):
            if i in matching_indices:
                ...
                self.assertEqual(
                    message.message_type, self.protocol.default_message_type, msg=str(i)
                )
